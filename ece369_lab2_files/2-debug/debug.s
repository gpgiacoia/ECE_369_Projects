...
#  Max Score: 9 points
#
# Students: Carson Keegan, Giuseppe Pongelupe, Leo Dickinson
# Percent Effort: Carson Keegan - 33%, Giuseppe Pongelupe - 33%, Leo Dickinson - 33%
#
...
#	$t0 holds onto the current char within the string as the program loops through the string values. $t1 continually increments allowing for $t0 to fetch the next char in the string.
#	$t2 is incremented when a matching character is found and holds the count value. $t3 is initialized with the character that we are trying to count in the string and is used for comparison with $t0.
# 2. Currently, the program is stuck in an infinite loop. Make use of 
#    breakpoints to locate, and correct the error.
#		The error has been fixed, $t1 was not incrementing properly leading to the infinite loop.
...
con:	
    ...
    addi     $t1, $t1, 1	    # increase indexing register $t1, Fixed $t1 incrementing
