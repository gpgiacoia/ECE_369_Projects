...
#	Register $t0 serves as the current address of the value that we are using in the array. Firstly in main: it is set to be the address of the first 
#	element in the array, then with every loop interation it is incremented by a unit of 4 (bytes) which goes to the next element in the array. 
#	Meaning, it serves as the address of the current element beeing used
#
.data                   	# Put Global Data here
N:      .word 3 		# 'N' is the address which contains the loop count, 3
